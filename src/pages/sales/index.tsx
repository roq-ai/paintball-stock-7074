...
import { getSales, deleteSalesById } from 'apiSdk/sales';
import { SalesInterface } from 'interfaces/sales';

type ColumnType = ColumnDef<SalesInterface, unknown>;
...
export function SalesListPage(props: SalesListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SalesInterface>>(
  ...
  const handleView = (row: SalesInterface) => {
    if (hasAccess('sales', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/sales/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'number_of_players', header: 'number_of_players', accessorKey: 'number_of_players' },
    { id: 'paintballs_per_player', header: 'paintballs_per_player', accessorKey: 'paintballs_per_player' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          ...
          {hasAccess('sales', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="sales" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Sales
        </Text>
        {hasAccess('sales', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
